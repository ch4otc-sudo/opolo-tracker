...
function toPayload(form) {
  return {
    ...
    comments: form.comments !== "" ? parseInt(form.comments) : 0,
    shares: form.shares !== "" ? parseInt(form.shares) : 0,
    reach: form.reach !== "" ? parseInt(form.reach) : 0,
...
export default function PostTracker() {
  ...
  async function handleSubmit() {
    ...
    try {
      if (editingId !== null) {
        const updated = await updatePost(editingId, toPayload(form));
  ...
  return (
    <div style={{ fontFamily: "monospace", padding: "20px", maxWidth: "900px", margin: "0 auto" }}>
      ...
      <fieldset>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: "8px", marginTop: "6px" }}>
          {["likes", "comments", "shares", "reach", "impressions"].map((field) => (
            <label key={field}>
              {field.charAt(0).toUpperCase() + field.slice(1)}<br />
              <input type="number" name={field} value={form[field]} onChange={handleChange} style={{ width: "100%" }} min="0" />
            </label>
          ))}
        </div>
